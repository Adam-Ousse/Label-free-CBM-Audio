...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate CREMA-D AST checkpoint")
    parser.add_argument(
        "--model_id",
        type=str,
        default="saved_models/ast_cremad/test/best_model",
    ...
    parser.add_argument("--expected_num_labels", type=int, default=6)
...
def main():
    ...
    if not os.path.exists(manifest_path):
        raise FileNotFoundError("Missing CREMA-D manifest for split '{}': {}".format(args.split, manifest_path))

    dataset = data_utils.get_audio_dataset("cremad", split=args.split, manifest_path=manifest_path)
    ...
    if int(classifier.num_labels) != int(args.expected_num_labels):
        raise ValueError(
            "Model outputs {} classes, expected {} for CREMA-D".format(
                classifier.num_labels,
                args.expected_num_labels,
            )
        )

    classes = data_utils.get_dataset_classes("cremad")
