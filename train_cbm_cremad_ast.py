...
def _manifest_path(split):
    return os.path.join("data", "cremad", "manifests", "{}.jsonl".format(split))
...
def _validate_inputs(args):
    ...
    for split in (args.train_split, args.val_split, args.test_split):
        ...
        if not os.path.exists(manifest):
            raise FileNotFoundError("Missing CREMA-D manifest for split '{}': {}".format(split, manifest))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train CBM on CREMA-D with train/val/test protocol")
    parser.add_argument("--concept_set", type=str, default="data/concept_sets/cremad_filtered_qwen.txt")
...
def main():
    ...
    if not args.allow_custom_splits:
        if args.train_split != "train":
            raise ValueError("Default CREMA-D protocol requires train_split='train'")
        if args.val_split != "val":
            raise ValueError("Default CREMA-D protocol requires val_split='val'")
        if args.test_split != "test":
            raise ValueError("Default CREMA-D protocol requires test_split='test'")

    _validate_inputs(args)

    print(
        "CREMA-D CBM splits -> train: {} val: {} test: {}".format(
            args.train_split,
            args.val_split,
            args.test_split,
        )
