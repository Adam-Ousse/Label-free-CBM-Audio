#!/usr/bin/env python3
"""Build and verify the external artifact bundle used by the paper.
...
compact evaluation reports used by the paper. Upload the whole directory to
...
After download, run:
    ...
    python scripts/build_google_drive_bundle.py --output /path/to/bundle --verify-only
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="audio_lf_cbm_bundle_", dir=output.parent) as tmp:
        ...
        manifest = {
            "format_version": 1,
            "description": "Canonical artifacts for Audio LF-CBM paper results",
            "models": models,
            "files": files,
