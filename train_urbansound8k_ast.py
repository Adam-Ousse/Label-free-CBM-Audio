...
def parse_args():
    parser = argparse.ArgumentParser(description="Train AST on UrbanSound8K with fixed folds")
    parser.add_argument("--base_model_id", type=str, default="MIT/ast-finetuned-audioset-10-10-0.4593")
    parser.add_argument("--urbansound8k_root", type=str, default="data/urbansound8k/raw")
    parser.add_argument("--manifest_root", type=str, default="data/urbansound8k")
    parser.add_argument("--output_dir", type=str, default="saved_models/ast_urbansound8k")
    ...
    parser.add_argument("--wandb_project", type=str, default="lf-cbm-audio-urbansound8k")
    parser.add_argument("--wandb_entity", type=str, default=None)
    parser.add_argument("--run_name", type=str, default="ast-urbansound8k")
...
def ensure_urbansound8k_manifests(args):
    required = [
    ...
    if args.no_prepare_manifests:
        raise FileNotFoundError(
            "Missing UrbanSound8K metadata artifacts. Run data/prepare_urbansound8k.py or remove --no_prepare_manifests. First missing: {}".format(
                missing[0]
            )
        )
    ...
    print("Preparing UrbanSound8K manifests via:", " ".join(cmd))
    subprocess.run(cmd, check=True)


def load_label_maps_from_metadata():
    mappings = data_utils.get_audio_label_mappings("urbansound8k")
    idx_to_label = mappings.get("idx_to_label", {})
    if len(idx_to_label) != 10:
        raise ValueError("Expected 10 labels in data/urbansound8k/idx_to_label.json")
...
def create_loaders(args, device):
    ...
    train_ds = data_utils.get_audio_dataset("urbansound8k", split="train", manifest_path=train_manifest)
    val_ds = data_utils.get_audio_dataset("urbansound8k", split="val", manifest_path=val_manifest)
    test_ds = data_utils.get_audio_dataset("urbansound8k", split="test", manifest_path=test_manifest)
...
def build_model(base_model_id, device, id2label, label2id):
    ...
    if old_out != 10:
        model.classifier.dense = torch.nn.Linear(in_features, 10).to(device)
        print("reinitialized classifier head: {} -> 10 classes".format(old_out))
    else:
        print("classifier head already has 10 classes")
...
def main():
    ...
    ensure_urbansound8k_manifests(args)
    ...
    fold_dir = os.path.join(args.output_dir, _split_tag(args.test_split))
