#!/usr/bin/env python3
"""Sweep the number of kept concepts by truncating the existing ranked ESC-50 concept file."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ESC-50 concept-set-size sweep")
...
def resolve_splits(args: argparse.Namespace):
    if args.allow_custom_splits:
        ...
        if args.test_fold != 1:
            raise ValueError("Default concept-size sweep uses test_fold=1 unless --allow_custom_splits is set")
        if args.val_fold is not None and args.val_fold != 2:
            raise ValueError("Default concept-size sweep uses val_fold=2 unless --allow_custom_splits is set")
        test_fold = 1
        val_fold = 2
...
def load_concepts(path: Path) -> list[str]:
    if not path.exists():
        raise FileNotFoundError(f"Concept set file not found: {path}")
    with path.open("r", encoding="utf-8") as f:
        concepts = [line.strip() for line in f if line.strip()]
    if not concepts:
        raise ValueError(f"Concept set is empty: {path}")
    return concepts


def build_settings():
    return [
        {"run_name": "top50", "max_concepts": 50},
        {"run_name": "top100", "max_concepts": 100},
        {"run_name": "top200", "max_concepts": 200},
    ]
...
def main() -> None:
    ...
    concepts = load_concepts(Path(args.concept_set))
    for setting in settings:
        if setting["max_concepts"] > len(concepts):
            raise ValueError(
                f"Requested top-{setting['max_concepts']} concepts, but {args.concept_set} only has {len(concepts)} entries"
            )

    experiment_name = "concept_set_size"
    ...
    for index, setting in enumerate(tqdm(settings, desc=experiment_name, unit="run"), start=1):
        ...
        train_args = build_train_namespace(
            ...
            max_concepts=setting["max_concepts"],
            run_name=setting["run_name"],
        )
        log_run_start(logger, index, len(settings), setting["run_name"], f"max_concepts={setting['max_concepts']}")
        commands.append(f"{setting['run_name']}: max_concepts={setting['max_concepts']}")
        ...
        row.update(
            {
                ...
                "run_name": setting["run_name"],
                "max_concepts_requested": setting["max_concepts"],
    ...
    note = "Concept-set size is controlled by truncating the existing concept list in file order."
