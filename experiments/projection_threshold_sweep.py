#!/usr/bin/env python3
"""Sweep the projection-faithfulness threshold used to prune weak concepts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ESC-50 projection threshold sweep")
...
def resolve_splits(args: argparse.Namespace):
    if args.allow_custom_splits:
        ...
        if args.test_fold != 1:
            raise ValueError("Default threshold sweep uses test_fold=1 unless --allow_custom_splits is set")
        if args.val_fold is not None and args.val_fold != 2:
            raise ValueError("Default threshold sweep uses val_fold=2 unless --allow_custom_splits is set")
        test_fold = 1
        val_fold = 2
...
def build_settings():
    return [
        {"run_name": "threshold_035", "projection_threshold": 0.35},
        {"run_name": "threshold_045", "projection_threshold": 0.45},
        {"run_name": "threshold_055", "projection_threshold": 0.55},
    ]
...
def main() -> None:
    ...
    experiment_name = "projection_threshold"
    ...
    for index, setting in enumerate(tqdm(settings, desc=experiment_name, unit="run"), start=1):
        ...
        train_args = build_train_namespace(
            ...
            projection_threshold=setting["projection_threshold"],
            run_name=setting["run_name"],
        )
        log_run_start(logger, index, len(settings), setting["run_name"], f"projection_threshold={setting['projection_threshold']}")
        commands.append(f"{setting['run_name']}: projection_threshold={setting['projection_threshold']}")
    ...
    note = "The threshold is the interpretability cutoff applied after projection learning." \
        " Lower values keep more concepts; higher values prune more aggressively."
