#!/usr/bin/env python3
"""Compare cosine and cosine-cubed projection objectives for ESC-50 LF-CBM training."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ESC-50 projection objective ablation")
...
def resolve_splits(args: argparse.Namespace):
    if args.allow_custom_splits:
        ...
        if args.test_fold != 1:
            raise ValueError("Default objective sweep uses test_fold=1 unless --allow_custom_splits is set")
        if args.val_fold is not None and args.val_fold != 2:
            raise ValueError("Default objective sweep uses val_fold=2 unless --allow_custom_splits is set")
        test_fold = 1
        val_fold = 2
...
def build_settings():
    return [
        {"run_name": "cosine", "similarity_objective": "cosine"},
        {"run_name": "cosine_cubed", "similarity_objective": "cosine_cubed"},
    ]
...
def main() -> None:
    ...
    experiment_name = "similarity_objective"
    ...
    for index, setting in enumerate(tqdm(settings, desc=experiment_name, unit="run"), start=1):
        ...
        train_args = build_train_namespace(
            ...
            similarity_objective=setting["similarity_objective"],
            run_name=setting["run_name"],
        )
        log_run_start(logger, index, len(settings), setting["run_name"], f"similarity_objective={setting['similarity_objective']}")
        commands.append(f"{setting['run_name']}: similarity_objective={setting['similarity_objective']}")
    ...
    note = "The only varied knob is the projection similarity target used before concept filtering."
